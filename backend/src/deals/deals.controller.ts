...
export class DealsController {
  ...
  async createFromAddon(
    ...
    const canonicalPayload = JSON.stringify({
      ...
      body: body.body || '',
      sender: body.sender || '',
      timestamp: addonTimestamp, // 👈 Optimized: Using raw header string for exact signature match
    });

    const normalizedSignature = addonSignature?.trim() || '';
    const received = Buffer.from(normalizedSignature, 'utf8');

    const acceptedSecrets = [this.configService.get<string>('ADDON_SECRET_KEY')].filter(Boolean);
    if (acceptedSecrets.length === 0) {
      throw new InternalServerErrorException('ADDON_SECRET_KEY is not configured');
    }
